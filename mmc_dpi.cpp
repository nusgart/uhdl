...
do_mmc_setup(struct state_s *s)
{
	...
	printf("mmc_dpi: using disk image '%s'\n", disk_image_filename);
...
do_mmc_read(struct state_s *s)
{
	...
	printf("mmc_dpi: buffer %03o %03o %03o %03o (%02x %02x %02x %02x)\n",
...
{
	if (debug) printf("mmc_dpi: %d (lba %d) (write)\n",
			  s->lba*s->blksize, s->lba);

	printf("mmc_dpi: write prep\n");
...
do_mmc_write_done(struct state_s *s)
{
	...
	printf("mmc_dpi: %d (lba %d), (write done)\n",
	       s->lba*s->blksize, s->lba);
...
void mmc_resp(struct state_s *s, int len, char *resp, int delay)
{
	...
	if (debug)
		printf("mmc_dpi: queue resp; size %d, delay %d\n", len, delay);
}
...
void mmc_dpi(int m_di, int* m_do, int m_cs, int m_sclk)
{
	struct state_s *s;
	...
	if (debug > 1 && (sclk_pos_edge || sclk_neg_edge))
		printf("mmc_dpi: sclk_bit %d, sclk_edge %c\n",
	...
	if (debug > 1) {
		if (sclk_pos_edge)
			printf("mmc_dpi: sclk pos edge, di %d\n", di_asserted);
		if (sclk_neg_edge)
			printf("mmc_dpi: sclk neg edge, di %d\n", di_asserted);
		if (cs_edge && cs_asserted != 0)
			printf("mmc_dpi: cs asserted\n");
		if (cs_edge && cs_asserted == 0)
			printf("mmc_dpi: cs deasserted\n");
	}
...
#define CMD24	0x58
		...
		if (debug > 1 && s->mode == M_CMD)
			printf("mmc_dpi: mode%d bits %d reg %12llx\n", s->mode, s->cmd_bits, s->cmd_reg);
		...
		case M_CMD:
			if (s->cmd_bits == 48) {
				uint8_t parity = s->cmd_reg & 0xff;
				uint8_t cmd = (s->cmd_reg >> 40) & 0xff;

				if (debug)
					printf("mmc_dpi: cmd %02x %llx\n", cmd, s->cmd_reg);
				...
				case CMD16:
					...
					if (debug)
						printf("mmc_dpi: CMD16 blksize=%d\n", s->blksize);
					mmc_set_mode(s, M_CMD_RESP);
					mmc_resp_r1(s, 0, 0);
					break;
				case CMD17:
					if (debug)
						printf("mmc_dpi: CMD17\n");
	...
	if (sclk_pos_edge && !cs_asserted && di_asserted) {
		s->unselected_count++;
		if (debug > 1) printf("mmc_dpi: unselected_count %d\n", s->unselected_count);
		if (s->unselected_count >= 74) {
			if (s->mode != M_IDLE)
				printf("mmc_dpi: going idle\n");
		...
		if ((sclk_neg_edge && cs_asserted) || (cs_edge && cs_asserted)) {
			if (s->resp_size == 0) {
				...
				if (s->resp_delay == 0) {
					...
					if (debug > 1)
						printf("mmc_dpi: resp, index/size/bit=%d/%d/%d, do=%d\n",
